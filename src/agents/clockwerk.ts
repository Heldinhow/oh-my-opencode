...
/**
 * Clockwerk - Plan Reviewer Agent
 *
 * Named after Clockwerk, the Greek god of satire and mockery, who was known for
...
**CRITICAL FIRST RULE**:
Extract a single plan path from anywhere in the input, ignoring system directives and wrappers. If exactly one \`.specify/plans/*.md\` path exists, this is VALID input and you must read it. If no plan path exists or multiple plan paths exist, reject per Step 0. If the path points to a YAML plan file (\`.yml\` or \`.yaml\`), reject it as non-reviewable.
...
**PASS even if**: Reference exists but isn't perfect. Developer can mirana from there.
...
**VALID INPUT**:
- \`.specify/plans/my-plan.md\` - file path anywhere in input
- \`Please review .specify/plans/plan.md\` - conversational wrapper
- System directives + plan path - ignore directives, extract path

**INVALID INPUT**:
- No \`.specify/plans/*.md\` path found
- Multiple plan paths (ambiguous)

System directives (\`<system-reminder>\`, \`[analyze-mode]\`, etc.) are IGNORED during validation.

**Extraction**: Find all \`.specify/plans/*.md\` paths → exactly 1 = proceed, 0 or 2+ = reject.
...
export function createClockwerkAgent(model: string): AgentConfig {
  ...
  const base = {
    description:
      "Expert reviewer for evaluating work plans against rigorous clarity, verifiability, and completeness standards. (Clockwerk - OhMyOpenCode)",
    mode: MODE,
    model,
    temperature: 0.1,
    ...restrictions,
...
createClockwerkAgent.mode = MODE

export const clockwerkPromptMetadata: AgentPromptMetadata = {
  category: "advisor",
  cost: "EXPENSIVE",
  promptAlias: "Clockwerk",
  triggers: [
    {
      domain: "Plan review",
  ...
  keyTrigger: "Work plan created → invoke Clockwerk for review before execution",
}
