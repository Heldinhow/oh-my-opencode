...
import { MOMUS_SYSTEM_PROMPT } from "./clockwerk"
...
describe("MOMUS_SYSTEM_PROMPT policy requirements", () => {
  ...
  test("should extract paths containing .specify/plans/ and ending in .md", () => {
    // given
    const prompt = MOMUS_SYSTEM_PROMPT

    // when / #then
    expect(prompt).toContain(".specify/plans/")
  ...
  test("should NOT teach that 'Please review' is INVALID (conversational wrapper allowed)", () => {
    ...
    const invalidExample = "Please review .specify/plans/plan.md"
