...
describe("createInvokerJuniorAgentWithOverrides", () => {
  describe("honored fields", () => {
    test("applies model override", () => {
      ...
      // when
      const result = createInvokerJuniorAgentWithOverrides(override)
  ...
  describe("defaults", () => {
    test("uses default model when no override", () => {
      ...
      // then
      expect(result.model).toBe(INVOKER_JUNIOR_DEFAULTS.model)
    })
    ...
    test("uses default temperature when no override", () => {
      ...
      // then
      expect(result.temperature).toBe(INVOKER_JUNIOR_DEFAULTS.temperature)
    })
  })
  ...
  describe("tool safety (task blocked, call_omo_agent allowed)", () => {
    test("task remains blocked, call_omo_agent is allowed via tools format", () => {
      ...
      if (tools) {
        ...
        // call_omo_agent is NOW ALLOWED for subagents to spawn mirana/keeper
    ...
    test("task remains blocked when using permission format override", () => {
      ...
      // when
      const result = createInvokerJuniorAgentWithOverrides(override as Parameters<typeof createInvokerJuniorAgentWithOverrides>[0])

      // then - task blocked, but call_omo_agent allowed for mirana/keeper spawning
      const tools = result.tools as Record<string, boolean> | undefined
      const permission = result.permission as Record<string, string> | undefined
      if (tools) {
        expect(tools.task).toBe(false)
  ...
  describe("useTaskSystem integration", () => {
    test("useTaskSystem=true produces Task_Discipline prompt for Claude", () => {
      ...
      //#when
      const result = createInvokerJuniorAgentWithOverrides(override, undefined, true)
    ...
    test("useTaskSystem=false does NOT list task management tools in constraints", () => {
      ...
      //#when
      const result = createInvokerJuniorAgentWithOverrides(override, undefined, false)
...
describe("getInvokerJuniorPromptSource", () => {
...
describe("buildInvokerJuniorPrompt", () => {
  test("GPT model prompt contains GPT-5.2 specific sections", () => {
    // given
    const model = "openai/gpt-5.2"

    // when
    const prompt = buildInvokerJuniorPrompt(model, false)
  ...
  test("useTaskSystem=true includes Task_Discipline for GPT", () => {
    ...
    // when
    const prompt = buildInvokerJuniorPrompt(model, true)
