...
import { createTinkerMdOnlyHook } from "./index"
import { MESSAGE_STORAGE } from "../../features/hook-message-injector"

describe("tinker-md-only", () => {
  const TEST_SESSION_ID = "test-session-tinker"
  ...
  describe("agent name matching", () => {
    test("should enforce md-only restriction for exact tinker agent name", async () => {
      //#given
    ...
    test("should enforce md-only restriction for uppercase TINKER", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "TINKER")
      const hook = createTinkerMdOnlyHook(createMockPluginInput())
      const input = {
    ...
    test("should not enforce restriction for non-Tinker agent", async () => {
      //#given
      setupMessageStorage(TEST_SESSION_ID, "invoker")
      const hook = createTinkerMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
        sessionID: TEST_SESSION_ID,
  ...
  describe("with Tinker agent in message storage", () => {
    ...
    test("should allow Tinker to write .md files inside .specify/", async () => {
      // given
      const hook = createTinkerMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      const output = {
        args: { filePath: "/tmp/test/.specify/plans/work-plan.md" },
      }
    ...
    test("should inject workflow reminder when Tinker writes to .specify/plans/", async () => {
      // given
      const hook = createTinkerMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      // then
      expect(output.message).toContain("TINKER MANDATORY WORKFLOW REMINDER")
      expect(output.message).toContain("INTERVIEW")
      expect(output.message).toContain("RUBICK CONSULTATION")
      expect(output.message).toContain("CLOCKWERK REVIEW")
    })

    test("should NOT inject workflow reminder for .specify/drafts/", async () => {
      // given
      const hook = createTinkerMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
      ...
      const output: { args: Record<string, unknown>; message?: string } = {
        args: { filePath: "/tmp/test/.specify/drafts/notes.md" },
      }
    ...
    test("should block Tinker from writing .md files outside .specify/", async () => {
      // given
      const hook = createTinkerMdOnlyHook(createMockPluginInput())
      const input = {
        tool: "Write",
  ...
  describe("boulder state priority over message files (fixes #927)", () => {
    ...
    const BOULDER_FILE = join(BOULDER_DIR, ".specify", "boulder.json")

    beforeEach(() => {
      mkdirSync(join(BOULDER_DIR, ".specify"), { recursive: true })
    })
    ...
    //#given session was started with tinker (first message), but /start-work set boulder agent to axe
    //#when user types "continue" after interruption (memory cleared, falls back to message files)
    //#then should use boulder state agent (axe), not message file agent (tinker)
    test("should prioritize boulder agent over message file agent", async () => {
      // given - tinker in message files (from /plan)
      setupMessageStorage(TEST_SESSION_ID, "tinker")
      
      // given - axe in boulder state (from /start-work)
      ...
      const hook = createTinkerMdOnlyHook({
        client: {},
      ...
      // when / then - should NOT block because boulder says axe, not tinker
      await expect(
        hook["tool.execute.before"](input, output)
      ).resolves.toBeUndefined()
    })

    test("should use tinker from boulder state when set", async () => {
      // given - axe in message files (from some other agent)
      setupMessageStorage(TEST_SESSION_ID, "axe")
      
      // given - tinker in boulder state (edge case, but should honor it)
      writeFileSync(BOULDER_FILE, JSON.stringify({
        active_plan: "/test/plan.md",
        started_at: new Date().toISOString(),
        session_ids: [TEST_SESSION_ID],
      ...
      // when / then - should block because boulder says tinker
      await expect(
        hook["tool.execute.before"](input, output)
      ).rejects.toThrow("can only write/edit .md files")
    })
    ...
    test("should fall back to message files when session not in boulder", async () => {
      ...
      // when / then - should block because falls back to message files (tinker)
      await expect(
        hook["tool.execute.before"](input, output)
      ).rejects.toThrow("can only write/edit .md files")
    })
  ...
  describe("cross-platform path validation", () => {
    beforeEach(() => {
     ...
     test("should allow Windows-style backslash paths under .specify/", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "tinker")
       const hook = createTinkerMdOnlyHook(createMockPluginInput())
       const input = {
       ...
       const output = {
         args: { filePath: ".specify\\plans\\work-plan.md" },
       }
     ...
     test("should allow mixed separator paths under .specify/", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "tinker")
       const hook = createTinkerMdOnlyHook(createMockPluginInput())
       const input = {
     ...
     test("should block paths outside workspace root even if containing .invoker", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "tinker")
       const hook = createTinkerMdOnlyHook(createMockPluginInput())
       const input = {
       ...
       const output = {
         args: { filePath: "/other/project/.specify/plans/x.md" },
       }
     ...
     test("should allow nested .invoker directories (ctx.directory may be parent)", async () => {
       ...
       const output = {
         args: { filePath: "src/.specify/plans/x.md" },
       }

       // when / #then - should allow because .invoker is in path
       await expect(
         hook["tool.execute.before"](input, output)
       ).resolves.toBeUndefined()
     })
     ...
     test("should block path traversal attempts", async () => {
       ...
       const output = {
         args: { filePath: ".specify/../secrets.md" },
       }
    ...
    test("should allow case-insensitive .SPECIFY directory", async () => {
     ...
     test("should allow nested project path with .invoker (Windows real-world case)", async () => {
       // given - simulates when ctx.directory is parent of actual project
       // User reported: xauusd-dxy-plan\.specify\drafts\supabase-email-templates.md
       setupMessageStorage(TEST_SESSION_ID, "tinker")
       const hook = createTinkerMdOnlyHook(createMockPluginInput())
       const input = {
         tool: "Write",
       ...
       const output = {
         args: { filePath: "xauusd-dxy-plan\\.specify\\drafts\\supabase-email-templates.md" },
       }
     ...
     test("should allow nested project path with mixed separators", async () => {
       ...
       const output = {
         args: { filePath: "my-project/.specify\\plans/task.md" },
       }
     ...
     test("should block nested project path without .invoker", async () => {
       // given
       setupMessageStorage(TEST_SESSION_ID, "tinker")
       const hook = createTinkerMdOnlyHook(createMockPluginInput())
       const input = {
