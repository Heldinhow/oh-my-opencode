...
describe("invoker-task-retry", () => {
  ...
  describe("detectDelegateTaskError", () => {
    ...
    it("should detect unknown agent error", () => {
      const output = '[ERROR] Unknown agent: "fake-agent". Available agents: mirana, keeper, oracle'
  ...
  describe("buildRetryGuidance", () => {
    ...
    it("should provide fix for unknown agent with available list", () => {
      const errorInfo = { 
        ...
        originalOutput: '[ERROR] Unknown agent: "fake". Available agents: mirana, oracle' 
      }
      
      const guidance = buildRetryGuidance(errorInfo)
      
      expect(guidance).toContain("mirana")
