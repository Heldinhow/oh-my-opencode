...
export async function promptInstallConfig(detected: DetectedConfig): Promise<InstallConfig | null> {
  ...
  const claude = await selectOrCancel<ClaudeSubscription>({
    ...
    options: [
      ...
      { value: "max20", label: "Yes (max20 mode)", hint: "Full power with Claude Sonnet 4.5 for Keeper" },
    ],
  ...
  const zaiCodingPlan = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      { value: "yes", label: "Yes", hint: "Fallback for Keeper and Multimodal Looker" },
    ],
  ...
  const kimiForCoding = await selectOrCancel({
    ...
    options: [
      { value: "no", label: "No", hint: "Will use other configured providers" },
      { value: "yes", label: "Yes", hint: "Kimi K2.5 for Invoker/Tinker fallback" },
    ],
