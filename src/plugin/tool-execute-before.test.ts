...
describe("createToolExecuteBeforeHandler", () => {
  ...
  describe("task tool subagent_type normalization", () => {
    ...
    test("sets subagent_type to invoker-junior when category is provided without subagent_type", async () => {
      //#given
      ...
      //#then
      expect(output.args.subagent_type).toBe("invoker-junior")
    })
    ...
    test("sets subagent_type to invoker-junior when category provided with different subagent_type", async () => {
      //#given
      const ctx = createCtxWithSessionMessages()
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
    ...
    test("resolves subagent_type from session first message when session_id provided without subagent_type", async () => {
      ...
      const ctx = createCtxWithSessionMessages([
        { info: { role: "user" } },
        { info: { role: "assistant", agent: "mirana" } },
      ...
      //#then
      expect(output.args.subagent_type).toBe("mirana")
    })
    ...
    test("preserves subagent_type when session_id is provided with explicit subagent_type", async () => {
      ...
      const handler = createToolExecuteBeforeHandler({ ctx, hooks: emptyHooks })
      const input = { tool: "task", sessionID: "ses_123", callID: "call_1" }
      const output = { args: { session_id: "ses_abc123", subagent_type: "mirana", description: "Continue mirana" } as Record<string, unknown> }
