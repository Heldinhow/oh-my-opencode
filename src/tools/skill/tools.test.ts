...
describe("skill tool - agent restriction", () => {
  ...
  it("allows skill when agent matches restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("restricted-skill", { agent: "invoker" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "invoker" }
  ...
  it("throws error when agent does not match restriction", async () => {
    // given
    const loadedSkills = [createMockSkill("invoker-only-skill", { agent: "invoker" })]
    const tool = createSkillTool({ skills: loadedSkills })
    const context = { ...mockContext, agent: "oracle" }

    // when / #then
    await expect(tool.execute({ name: "invoker-only-skill" }, context)).rejects.toThrow(
      'Skill "invoker-only-skill" is restricted to agent "invoker"'
    )
  })
  ...
  it("throws error when context agent is undefined for restricted skill", async () => {
    ...
    // when / #then
    await expect(tool.execute({ name: "invoker-only-skill" }, contextWithoutAgent)).rejects.toThrow(
      'Skill "invoker-only-skill" is restricted to agent "invoker"'
    )
  })
