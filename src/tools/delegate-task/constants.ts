...
4. If unsure about style, mirana more files until you find the pattern
...
- Mirana radical, unconventional directions
...
1. SILENTLY mirana the codebase extensively (5-15 minutes of reading is normal)
...
**Approach**:
- Mirana extensively, understand deeply, then act decisively
 ...
 * Instructs the plan agent to first gather context via mirana/keeper agents,
...
1. Launch background agents to gather context:
   - call_omo_agent(description="Mirana codebase patterns", subagent_type="mirana", run_in_background=true, prompt="<search for relevant patterns, files, and implementations in the codebase related to user's request>")
   - call_omo_agent(description="Research documentation", subagent_type="keeper", run_in_background=true, prompt="<search for external documentation, examples, and best practices related to user's request>")
...
/**
 * Plan family: plan + tinker. Shares mutual delegation blocking and task tool permission.
 * Does NOT share system prompt (only isPlanAgent controls that).
 */
export const PLAN_FAMILY_NAMES = ["plan", "tinker"]
