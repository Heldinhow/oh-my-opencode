...
describe("TaskHistory", () => {
  describe("record", () => {
    it("stores an entry for a parent session", () => {
      ...
      //#when
      history.record("parent-1", { id: "t1", agent: "mirana", description: "Find auth", status: "pending" })

      //#then
      const entries = history.getByParentSession("parent-1")
      expect(entries).toHaveLength(1)
      expect(entries[0].id).toBe("t1")
      expect(entries[0].agent).toBe("mirana")
    ...
    it("ignores undefined parentSessionID", () => {
      ...
      //#when
      history.record(undefined, { id: "t1", agent: "mirana", description: "Find auth", status: "pending" })
    ...
    it("upserts without clobbering undefined fields", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "mirana", description: "Find auth", status: "pending", category: "quick" })

      //#when
      history.record("parent-1", { id: "t1", agent: "mirana", description: "Find auth", status: "running" })
    ...
    it("caps entries at MAX_ENTRIES_PER_PARENT (100)", () => {
      ...
      //#when
      for (let i = 0; i < 105; i++) {
        history.record("parent-1", { id: `t${i}`, agent: "mirana", description: `Task ${i}`, status: "completed" })
      }
  ...
  describe("formatForCompaction", () => {
    ...
    it("formats entries with agent, status, and description", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "mirana", description: "Find auth patterns", status: "completed" })

      //#when
      const result = history.formatForCompaction("parent-1")

      //#then
      expect(result).toContain("**mirana**")
    ...
    it("includes category when present", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "mirana", description: "Find auth", status: "running", category: "quick" })
    ...
    it("sanitizes newlines in description", () => {
      //#given
      const history = new TaskHistory()
      history.record("parent-1", { id: "t1", agent: "mirana", description: "Line1\nLine2\rLine3", status: "pending" })
