...
> "If Claude Code does in 7 days what a human does in 3 months, Invoker does it in 1 hour. It just works until the task is done. It is a discipline agent." — B, Quant Researcher
...
Meet our main agent: Invoker (Opus 4.5 High). Below are the tools Invoker uses to keep that boulder rolling.
...
- Invoker's Teammates (Curated Agents)
  - Enigma: Autonomous deep worker, goal-oriented execution (GPT 5.2 Codex Medium) — *The Legitimate Craftsman*
  - Oracle: Design, debugging (GPT 5.2 Medium)
  - Frontend UI/UX Engineer: Frontend development (Gemini 3 Pro)
  - Keeper: Official docs, open source implementations, codebase exploration (Claude Sonnet 4.5)
  - Mirana: Blazing fast codebase exploration (Contextual Grep) (Claude Haiku 4.5)
- Full LSP / AstGrep Support: Refactor decisively.
- Todo Continuation Enforcer: Forces the agent to continue if it quits halfway. **This is what keeps Invoker rolling that boulder.**
- Comment Checker: Prevents AI from adding excessive comments. Code generated by Invoker should be indistinguishable from human-written code.
...
1. Invoker doesn't waste time hunting for files himself; he keeps the main agent's context lean. Instead, he fires off background tasks to faster, cheaper models in parallel to map the territory for him.
1. Invoker leverages LSP for refactoring; it's more deterministic, safer, and surgical.
1. When the heavy lifting requires a UI touch, Invoker delegates frontend tasks directly to Gemini 3 Pro.
1. If Invoker gets stuck in a loop or hits a wall, he doesn't keep banging his head—he calls GPT 5.2 for high-IQ strategic backup.
1. Working with a complex open-source framework? Invoker spawns subagents to digest the raw source code and documentation in real-time. He operates with total contextual awareness.
1. When Invoker touches comments, he either justifies their existence or nukes them. He keeps your codebase clean.
1. Invoker is bound by his TODO list. If he doesn't finish what he started, the system forces him back into "bouldering" mode. Your task gets done, period.
1. Honestly, don't even bother reading the docs. Just write your prompt. Include the 'ultrawork' keyword. Invoker will analyze the structure, gather the context, dig through external source code, and just keep bouldering until the job is 100% complete.
...
*Why "Legitimate"? When Anthropic blocked third-party access citing ToS violations, the community started joking about "legitimate" usage. Enigma embraces this irony—he's the craftsman who builds things the right way, methodically and thoroughly, without cutting corners.*

Enigma is inspired by [AmpCode's deep mode](https://ampcode.com)—autonomous problem-solving with thorough research before decisive action. He doesn't need step-by-step instructions; give him a goal and he'll figure out the rest.
...
- If you're on [1.0.132](https://github.com/sst/opencode/releases/tag/v1.0.132) or older, an OpenCode bug may break config.
  - [The fix](https://github.com/sst/opencode/pull/5040) was merged after 1.0.132—use a newer version.
    - Fun fact: That PR was discovered and fixed thanks to OhMyOpenCode's Keeper, Mirana, and Oracle setup.
